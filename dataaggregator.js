...
document.addEventListener("DOMContentLoaded", function (event) {
    ...
    document.getElementById("download").addEventListener("click", function (e) {
        ...
        if (obj.run) {
            ...
            download(`${obj.run}_wealth_distribution.csv`, serializeHist(obj.histogramWealth));
        }
    }, false);
...
function parseData(data) {
    ...
    // Post-hoc fix for Remove runs: eliminated agents are not counted in the
    // stored wealthDistribution, which causes the histogram denominator to shrink
    // over time and produces a spurious red band at the top of the panel.
    // We add removed agents back into bucket 0 at each tick so that all three
    // boundary condition panels share the same constant denominator (initialAgents).
    // Null agents are already counted in bucket 0 by datamanager.js, so there
    // is no double-counting here.
    const initialAgents = data[0].parameters.initialAgents;
    const isRemoveRun = data[0].parameters.removeDeadAgents === true;

    if (isRemoveRun) {
        for (let j = 0; j < ticks; j++) {
            let totalSurviving = 0;
            for (let i = 0; i < runs; i++) {
                totalSurviving += data[i].population[j];
            }
            const removed = (initialAgents * runs) - totalSurviving;
            histogramWealth[j][0] += removed;
        }
    }

    // Constant denominator: initialAgents * runs for every tick.
    // This keeps the colour scale comparable across Preserve, Null, and Remove.
    const constantTotal = [];
    for (let j = 0; j < ticks; j++) {
        constantTotal.push(initialAgents * runs);
    }
...
function drawData(runs) {
    ...
    drawHistogram(context, 0, 345, obj.histogramWealth, obj.constantTotal, "Wealth Distribution");
